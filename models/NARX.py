...
class NARX:
    def __init__(self, Xub, Xlb, Uub, Ulb, num_lift=2, weighting=0.96, Mub=None, Mlb=None):
        ...
        print('NARX sets up scale ranges and centers successfully!')
    ...
    def initialization(self, states, actions, costs=None):
        ...
        print('NARX sets up Random Fourier Features successfully!')
        ...
        if costs is None:
            ...
            NRMSE_NARX = 100*np.sqrt(sum(np.linalg.norm(error,axis=0)**2)) / np.sqrt(sum(np.linalg.norm(self.Y.T,axis=0)**2))
        else:
            error = self.YCY - np.matmul(self.Zeta,self.Theta)
            NRMSE_NARX = 100*np.sqrt(sum(np.linalg.norm(error,axis=0)**2)) / np.sqrt(sum(np.linalg.norm(self.YCY,axis=0)**2))
        print(NRMSE_NARX,"%")
        return self.Theta
