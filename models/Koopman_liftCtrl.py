...
class Koopman2:
    def __init__(self, Xub, Xlb, Uub, Ulb, num_lift=2, weighting=0.96, Mub=None, Mlb=None):
        ...
        print('Koopman2 sets up scale ranges and centers successfully!')
    ...
    def initialization(self, states, actions, costs=None):
        ...
        print('Koopman2 sets up Random Fourier Features successfully!')
        ...
        NRMSE_Koopman2 = 100*np.sqrt(sum(np.linalg.norm(error,axis=0)**2)) / np.sqrt(sum(np.linalg.norm(self.Y,axis=0)**2))
        print(NRMSE_Koopman2,"%")
