...
class MovingAnchor:
    def __init__(self, Xub, Xlb, Uub, Ulb, num_lift=2, weighting=0.96, Mub=None, Mlb=None):
        ...
        print('MovingAnchoring sets up scale ranges and centers successfully!')
    ...
    def initialization(self, states, actions, costs=None, Weights = None):
        ...
        print('MovingAnchoring sets up Random Fourier Features successfully!')
    ...
    def regression(self, states, actions, Weights, costs=None):
        ...
        NRMSE_MA = 100*np.sqrt(sum(np.linalg.norm(error,axis=0)**2)) / np.sqrt(sum(np.linalg.norm(self.Y.T,axis=0)**2))
        print(NRMSE_MA,"%")
