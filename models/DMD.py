...
class DMD:
    def __init__(self, Xub, Xlb, Uub, Ulb, num_lift=2, weighting=0.96, Mub=None, Mlb=None):
        ...
        print('DMD sets up scale ranges and centers successfully!')
    ...
    def initialization(self, states, actions, costs=None):
        ...
        NRMSE_DMD = 100*np.sqrt(sum(np.linalg.norm(error,axis=0)**2)) / np.sqrt(sum(np.linalg.norm(self.Y,axis=0)**2))
        print(NRMSE_DMD,"%")
    ...
    def update(self, states_x, states_y, actions, costs_x=None, costs_y= None):
        ...
        if np.linalg.det(self.G) < 0:
            ...
            print(min(rank))
            GG = np.linalg.inv(np.matmul(self.Zeta,self.Zeta.T) + 50*np.eye(np.size(self.Zeta,0)))
